...
export default {
  ...
  mounted() {
    ...
    let agentsPromise = fetch(this.apiUrls+`/agents`).then(response => {
        if (!response.ok) {
    ...
    let documentsPromise = fetch(this.apiUrls+`/documents`).then(response => {
        if (!response.ok) {
    ...
    let typesPromise = fetch(this.apiUrls+`/type_documents`).then(response => {
        if (!response.ok) {
    ...
    Promise.all([agentsPromise, documentsPromise, typesPromise]).then(([agentsData, documentsData, typesData]) => {
        this.agent = agentsData.find(agent => agent.agentID === id);
        if (!this.agent) {
            console.log("L'agent n'a pas été trouvé");
        }

        this.agent.documents = documentsData.filter(document => document.agentID === id);
        this.totalDocuments = this.agent.documents.length;

        this.documentTypes = typesData;
    }).catch(error => {
  ...
  methods: { 
        // Méthode pour naviguer vers la route précédente
        goBack() {
      this.$router.go(-1); // Navigue vers la route précédente
    },
